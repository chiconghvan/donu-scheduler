...
export default function JobsPage() {
  ...
  return <div className="page jobs-page"><div className="page__header"><h1 className="page__title"><CalendarClock size={18} /> Jobs</h1><div className="page__actions"><button className="btn btn--secondary" onClick={load}><RefreshCw size={14} /> Refresh</button><button className="btn btn--primary" onClick={startCreate}><Plus size={14} /> New Job</button></div></div>
    <div className="split-layout">
      ...
      <section className="panel"><div className="panel__header"><span>{mode === "edit" ? "Edit Job" : selected ? selected.name : "Job Detail"}</span>{mode === "edit" && <div className="page__actions"><button className="btn btn--secondary" type="button" onClick={() => setMode("view")} disabled={saving}>Cancel</button><button className="btn btn--primary" type="button" onClick={() => void save(false)} disabled={saving}>{saving ? "Saving..." : "Save Job"}</button></div>}</div><div className="panel__body">{mode === "edit" ? <JobForm form={form} setForm={setForm} scripts={scripts} onScriptChange={onScriptChange} inputs={inputs} setInputs={setInputs} schedule={schedule} setSchedule={setSchedule} profiles={profiles} openPicker={() => setPickerOpen(true)} validationErrors={validationErrors} saving={saving} showFooter={false} /> : selected ? <JobDetail job={selected} states={states} runs={runs} tab={tab} setTab={setTab} scripts={scripts} onEdit={() => startEdit(selected)} onDelete={() => void remove(selected)} onStop={async (id: string) => { await stopJobRun(id); await loadDetail(selected.id); }} /> : <EmptyState title="Select a job" description="Choose job from list or create new one." />}</div></section>
    </div>
