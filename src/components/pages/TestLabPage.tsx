    ...
    <div className="split-layout split-layout--40-60">
      <section className="panel"><div className="panel__header">Configuration</div><div className="panel__body form-grid">
        ...
        <div className="profile-filter-row">
          <div className="search-input profile-filter-row__search"><Search size={14} className="search-input__icon" /><input className="input" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search profiles" /></div>
          {groups.length > 0 && <select className="select profile-filter-row__group" value={group} onChange={(e) => setGroup(e.target.value)}><option value="all">All Groups</option>{groups.map((g) => <option key={g} value={g}>{g}</option>)}</select>}
        </div>
